...
public:
    Op_h2(
        ...
        const std::vector<std::vector<std::set<std::pair<int, int>>>>
            &inconsistent_facts,
        bool regression);
...
private:
    inline void push_pre(
        const vector<vector<unsigned>> &p_index, Variable *var, int val) {
        if (var->get_level() >= 0) {
            pre.push_back(p_index[var->get_level()][val]);
        }
    }

    inline void push_add(
        const vector<vector<unsigned>> &p_index, Variable *var, int val) {
        if (var->get_level() >= 0) {
...
public:
    ...
    int compute(
        ...
        vector<Operator>
            &operators, // not const because may be detected to be spurious
    ...
    int detect_unreachable_fluents(
...
extern bool compute_h2_mutexes(
    ...
    const vector<pair<Variable *, int>> &goal, int limit_seconds,
    bool disable_bw_h2);
