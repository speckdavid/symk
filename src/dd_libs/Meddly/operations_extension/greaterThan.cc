...
const MEDDLY::binary_opname *USER_OPS::GREATERTHAN;

namespace USER_OPS
{
class GreaterThan;
class GreaterThan_Opname;
} // namespace USER_OPS

class USER_OPS::GreaterThan : public MEDDLY::binary_operation
{
public:
  GreaterThan(const MEDDLY::binary_opname *code, MEDDLY::expert_forest *arg1,
...
protected:
                      ...
                      MEDDLY::node_handle &c)
  {
    ...
    {
      c = -1;
      cev = isgreater(aev, bev) ? 1 : 0;
      return true;
    }

    if (a == -1 && isgreaterequal(bev, aev)) {
      c = -1;
      cev = 0;
      return true;
    }
    ...
    if (0 == a || 0 == b)
    {
      std::cout << "ERROR => Zero edge pointer in greaterThan" << std::endl;
      return true;
    }
    return false;
  }
...
class USER_OPS::GreaterThan_Opname : public MEDDLY::binary_opname
{
public:
  GreaterThan_Opname() : binary_opname("EVGREATERTHAN") {}

  MEDDLY::binary_operation *buildOperation(MEDDLY::expert_forest *arg1,
                                           MEDDLY::expert_forest *arg2,
                                           MEDDLY::expert_forest *res) const
  {
    return new USER_OPS::GreaterThan(this, arg1, arg2, res);
  }
};

void USER_OPS::initializeGreaterThan()
{
  USER_OPS::GREATERTHAN = new USER_OPS::GreaterThan_Opname();
}
