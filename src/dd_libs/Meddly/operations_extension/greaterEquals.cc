...
const MEDDLY::binary_opname *USER_OPS::GREATEREQUALS;

namespace USER_OPS
{
class GreaterEquals;
class GreaterEquals_Opname;
} // namespace USER_OPS

class USER_OPS::GreaterEquals : public MEDDLY::binary_operation
{
public:
  GreaterEquals(const MEDDLY::binary_opname *code, MEDDLY::expert_forest *arg1,
...
protected:
                      ...
                      MEDDLY::node_handle &c)
  {
    ...
    {
      c = -1;
      cev = isgreaterequal(aev, bev) ? 1 : 0;
      return true;
    }
    ...
    if (a == -1 && isgreater(bev, aev)) {
      c = -1;
      cev = 0;
    }

    if (b == -1 && isgreaterequal(aev, bev)) {
      c = -1;
      cev = 1;
    }
    ...
    if (0 == a || 0 == b)
    {
      std::cout << "ERROR => Zero edge pointer in greaterEquals" << std::endl;
      return true;
    }
    return false;
  }
...
class USER_OPS::GreaterEquals_Opname : public MEDDLY::binary_opname
{
public:
  GreaterEquals_Opname() : binary_opname("EVGREATEREQUALS") {}

  MEDDLY::binary_operation *buildOperation(MEDDLY::expert_forest *arg1,
                                           MEDDLY::expert_forest *arg2,
                                           MEDDLY::expert_forest *res) const
  {
    return new USER_OPS::GreaterEquals(this, arg1, arg2, res);
  }
};

void USER_OPS::initializeGreaterEquals()
{
  USER_OPS::GREATEREQUALS = new USER_OPS::GreaterEquals_Opname();
}
