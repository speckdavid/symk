...
const MEDDLY::binary_opname *USER_OPS::LESSEQUALS;

namespace USER_OPS
{
class LessEquals;
class LessEquals_Opname;
} // namespace USER_OPS

class USER_OPS::LessEquals : public MEDDLY::binary_operation
{
public:
  LessEquals(const MEDDLY::binary_opname *code, MEDDLY::expert_forest *arg1,
...
protected:
                      ...
                      MEDDLY::node_handle &c)
  {
    ...
    {
      c = -1;
      cev = islessequal(aev, bev) ? 1 : 0;
      return true;
    }

    if (a == -1 && islessequal(aev, bev)) {
      c = -1;
      cev = 1;
      return true;
    }
    ...
    if (0 == a || 0 == b)
    {
      std::cout << "ERROR => Zero edge pointer in lessEquals" << std::endl;
      return true;
    }
    return false;
  }
...
class USER_OPS::LessEquals_Opname : public MEDDLY::binary_opname
{
public:
  LessEquals_Opname() : binary_opname("EVLESSEQUALS") {}

  MEDDLY::binary_operation *buildOperation(MEDDLY::expert_forest *arg1,
                                           MEDDLY::expert_forest *arg2,
                                           MEDDLY::expert_forest *res) const
  {
    return new USER_OPS::LessEquals(this, arg1, arg2, res);
  }
};

void USER_OPS::initializeLessEquals()
{
  USER_OPS::LESSEQUALS = new USER_OPS::LessEquals_Opname();
}
