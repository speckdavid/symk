...
const MEDDLY::binary_opname *USER_OPS::IMAGE;
...
class USER_OPS::Image : public MEDDLY::binary_operation
{
public:
  Image(const MEDDLY::binary_opname *code, MEDDLY::expert_forest *arg1,
...
protected:
               ...
               MEDDLY::node_handle b, float &cev, MEDDLY::node_handle &c)
  {
    ...
    // We skip to the top cube until it is leveled with our EVMDDs
    while(abstractLevels[index_topcube] > resultLevel) {
      index_topcube++;
      if (index_topcube >= abstractLevels) {
        
        MEDDLY::apply(USER_OPS::EVPLUS, )
      }
