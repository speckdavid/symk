...
class PlanReconstructor
{
  protected:
    ...
    Bdd bdd_for_zero_reconstruction(const Bdd &cut, int cost, std::shared_ptr<ClosedList> closed) const;
    ...
    void extract_all_plans(SymCut &sym_cut, bool fw, std::shared_ptr<ClosedList> closed, Plan plan);

    // Return wether a zero cost reconstruction step was necessary
    bool reconstruct_zero_action(SymCut &sym_cut, bool fw, std::shared_ptr<ClosedList> closed, const Plan& plan);
    bool reconstruct_cost_action(SymCut &sym_cut, bool fw, std::shared_ptr<ClosedList> closed, const Plan& plan);

  public:
    PlanReconstructor(UnidirectionalSearch* uni_search_fw, UnidirectionalSearch* uni_search_bw, std::shared_ptr<SymVariables> sym_vars, std::shared_ptr<StateRegistry> state_registry);

    // Resets found plans and desired_num_plans which are helper functions
    void reconstruct_plans(const SymCut &cut, size_t desired_num_plans, std::vector<Plan> &plans);
