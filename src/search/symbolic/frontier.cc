...
Result Frontier::prepare(int maxTime, int maxNodes, bool fw,
  ...
  if (!Smerge.empty()) {
    ...
    DEBUG_MSG(cout << " BUCKET MERGED: " << Smerge.size() << " "
                   << nodeCount(Smerge) << endl;);
  ...
  DEBUG_MSG(cout << " BUCKET PREPARED: " << S.size() << " " << nodeCount(S)
                 << endl;);
...
ResultExpansion Frontier::expand_zero(int maxTime, int maxNodes, bool fw) {
  ...
  DEBUG_MSG(cout << "  >> 0-cost step " << (fw ? " fw " : " bw ") << ": "
                 << nodesStep << " nodes "
                 << " done in " << image_time << endl;);
...
ResultExpansion Frontier::expand_cost(int maxTime, int maxNodes, bool fw) {
  ...
  DEBUG_MSG(cout << "  >> cost step" << (fw ? " fw " : " bw ") << ": "
                 << nodesStep << " nodes "
                 << " done in " << image_time << " total time: " << g_timer
                 << endl;);
