...
SymController::SymController(const Options &opts)
    : vars(make_shared<SymVariables>(opts)),
      mgrParams(opts), searchParams(opts), lower_bound(0), solution_registry(mgrParams.num_plans)
{
...
void SymController::setLowerBound(int lower, bool hard)
{
    ...
    {
        lower_bound = lower;
        std::cout << "BOUND: " << lower_bound << " < " << getUpperBound() << std::flush;
        if (searchParams.top_k) {
            solution_registry.construct_cheaper_solutions(lower);
        }
        std::cout << " [" << solution_registry.num_found_plans() << "/" << mgrParams.num_plans << " plans]" << std::flush;
