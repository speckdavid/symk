...
static plugins::TypedEnumPlugin<ConditionalEffectsTransitionType> _conditional_effect_transition_type__enum_plugin({
        ...
        {"var_based_conjunctive", "multiple conjunctive transition relations, one for each effect variable"},
        {"var_based_conjunctive_early_quantification", "same as var_based_conjunctive but with early quantification of variables"},
        {"eff_based_conjunctive", "multiple conjunctive transition relations, one for each conditional effect; adds auxiliary variables"},
        {"eff_based_conjunctive_early_quantification", "same as eff_based_conjunctive but with early quantification of variables"},
        {"dynamic", "tries to use variable-based for each transition relation, and if this does not succeed, falls back to effect-based, always with early quantification of variables"}
    });
...
ostream &operator<<(ostream &os, const ConditionalEffectsTransitionType &ce_type) {
    ...
    case ConditionalEffectsTransitionType::VAR_BASED_CONJUNCTIVE_EARLY_QUANTIFICATION:
    ...
    case ConditionalEffectsTransitionType::EFF_BASED_CONJUNCTIVE_EARLY_QUANTIFICATION:
        return os << "effect based conjunctive with early quantification";
    case ConditionalEffectsTransitionType::DYNAMIC:
        return os << "variable + effect based conjunctive with early quantification";
    default:
...
bool is_ce_transition_type_early_quantification(const ConditionalEffectsTransitionType &ce_type) {
    return ce_type == ConditionalEffectsTransitionType::VAR_BASED_CONJUNCTIVE_EARLY_QUANTIFICATION ||
           ce_type == ConditionalEffectsTransitionType::EFF_BASED_CONJUNCTIVE_EARLY_QUANTIFICATION ||
           ce_type == ConditionalEffectsTransitionType::DYNAMIC;
}

bool is_ce_transition_type_variable_based(const ConditionalEffectsTransitionType &ce_type) {
    return ce_type == ConditionalEffectsTransitionType::VAR_BASED_CONJUNCTIVE ||
           ce_type == ConditionalEffectsTransitionType::VAR_BASED_CONJUNCTIVE_EARLY_QUANTIFICATION ||
           ce_type == ConditionalEffectsTransitionType::DYNAMIC;
}
