...
// TODO: Disable Axiom Support for the effect based compilation. In general it
// should work but we need to test it.
...
static bool is_simple_disjunction_or_conjunction(const BDD &bdd) {
    ...
    return minterms == disjunction_states // Exact match for large values (inf)
           ...
           || fabs(minterms - disjunction_states) <
                  epsilon; // Disjunction: All but one minterm
}
#endif

ConjunctiveTransitionRelation::ConjunctiveTransitionRelation(
...
void ConjunctiveTransitionRelation::init_variable_based() {
    ...
    for (int ce_op_id :
         task->get_operators_beloning_to_parent(operator_id.get_index())) {
...
void ConjunctiveTransitionRelation::init_effect_based() {
    ...
    for (size_t eff_op_id = 0; eff_op_id < relevant_op_ids.size();
        ...
        add_frame_trs(
            effect_var, condition_aux_vars, single_effect_conditions,
            var_transitions);

        // Finalize transitions by setting operator ID and cost
        for (auto &transition : var_transitions) {
            assert(
                is_simple_disjunction_or_conjunction(transition.get_tr_BDD()));
...
bool ConjunctiveTransitionRelation::is_simple_condition(
    const ConditionsProxy &cond) {
    if (cond.size() > 1)
        return false;
...
void ConjunctiveTransitionRelation::add_precondition_trs(
    const PreconditionsProxy &pre,
...
void ConjunctiveTransitionRelation::add_aux_condition_trs(
    const EffectProxy &eff, int aux_var_id,
    vector<DisjunctiveTransitionRelation> &transitions) {
    // cond => aux == !cond OR aux (disjunction)
    transitions.emplace_back(sym_vars, task);
    auto &cond_aux_tr = transitions.back();
    cond_aux_tr.add_condition(eff.get_conditions());
    cond_aux_tr.set_tr_BDD(
        !cond_aux_tr.get_tr_BDD() + sym_vars->auxBDD(aux_var_id, 1));
    ...
    for (const auto &cond : eff.get_conditions()) {
        ...
        sym_vars->get_variable_value_bdds(
            pre_bdd_levels, cond_value, cond_value_bdds);
...
void ConjunctiveTransitionRelation::add_effect_trs(
    ...
    // We create one TR for each binary variable of the effect to obtain a pure
    // CNF
    const vector<int> &eff_bdd_levels = sym_vars->vars_index_eff(eff_var);
    vector<BDD> eff_value_bdds;
    sym_vars->get_variable_value_bdds(
        eff_bdd_levels, eff_value, eff_value_bdds);
...
void ConjunctiveTransitionRelation::add_frame_trs(
    const FactProxy &eff_variable,
    ...
    // If the condition does not hold (!cond), the effect variable (eff_var)
    // remains unchanged. This is encoded as two separate transitions:
...
void ConjunctiveTransitionRelation::init_exist_and_swap_vars() {
    ...
    for (const auto &tr : transitions) {
        ...
        all_swap_vars.insert(
            all_swap_vars.end(), tr.get_swap_vars().begin(),
            tr.get_swap_vars().end());
        all_swap_vars_p.insert(
            all_swap_vars_p.end(), tr.get_swap_vars_p().begin(),
            tr.get_swap_vars_p().end());
    }

    // Remove duplicates from all_swap_vars and all_swap_vars_p
    sort(all_swap_vars.begin(), all_swap_vars.end());
    all_swap_vars.erase(
        unique(all_swap_vars.begin(), all_swap_vars.end()),
        all_swap_vars.end());
    sort(all_swap_vars_p.begin(), all_swap_vars_p.end());
    all_swap_vars_p.erase(
        unique(all_swap_vars_p.begin(), all_swap_vars_p.end()),
        all_swap_vars_p.end());
...
void ConjunctiveTransitionRelation::prune_unused_exist_vars(
    vector<DisjunctiveTransitionRelation> &transitions) {
...
BDD ConjunctiveTransitionRelation::preimage(
    const BDD &from, int max_nodes) const {
    BDD res = from;
    res = res.SwapVariables(all_swap_vars, all_swap_vars_p);
    for (size_t tr_id = 0; tr_id < transitions.size(); ++tr_id) {
        const auto &tr = transitions[tr_id];
        res =
            res.AndAbstract(tr.get_tr_BDD(), exists_bw_vars[tr_id], max_nodes);
        if (res.IsZero()) {
            return res;
        }
    }
...
BDD ConjunctiveTransitionRelation::preimage(
    const BDD &from, const BDD &constraint_to, int max_nodes) const {
    BDD res = from;
    res = res.SwapVariables(all_swap_vars, all_swap_vars_p);
    res *= constraint_to;
    for (size_t tr_id = 0; tr_id < transitions.size(); ++tr_id) {
...
void ConjunctiveTransitionRelation::variable_based_transition_merging(
    int max_time, int max_nodes) {
    merge(sym_vars, transitions, conjunctive_tr_merge, max_time, max_nodes);
    init_exist_and_swap_vars();
}

// We try to merge per effect variable to faster get rid of aux variables and
// simulate the good working variable based partioning if feasible
void ConjunctiveTransitionRelation::effect_based_transition_merging(
    int max_time, int max_nodes) {
    for (auto &var_based_trs : var_based_effect_transitions) {
        merge(
            sym_vars, var_based_trs, conjunctive_tr_merge, max_time, max_nodes);
    ...
    assert(
        !is_monolithic() ||
        sym_vars->get_aux_variables_in_support(transitions[0].get_tr_BDD())
            .IsOne());
    assert(
        !is_monolithic() ||
        sym_vars->get_aux_variables_in_support(transitions[0].get_exists_vars())
            .IsOne());
    assert(
        !is_monolithic() ||
        sym_vars
            ->get_aux_variables_in_support(transitions[0].get_exists_bw_vars())
            .IsOne());
}

void ConjunctiveTransitionRelation::merge_transitions(
    int max_time, int max_nodes) {
    if (variable_based_splitting) {
        variable_based_transition_merging(max_time, max_nodes);
    } else {
...
const vector<DisjunctiveTransitionRelation> &
ConjunctiveTransitionRelation::get_transitions() const {
    return transitions;
}
...
int ConjunctiveTransitionRelation::nodeCount() const {
    return accumulate(
        ...
        [](int acc, const auto &tr) { return acc + tr.nodeCount(); });
}

const OperatorID &
ConjunctiveTransitionRelation::get_unique_operator_id() const {
    return operator_id;
}
