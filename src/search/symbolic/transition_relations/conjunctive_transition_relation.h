...
namespace symbolic {
/*
 * Represents a conjunctive transition relation (used for conditional effects)
 * with BDDs.
 */
class ConjunctiveTransitionRelation : public TransitionRelation {
protected:
    SymVariables *sym_vars; // To call basic BDD creation methods
    const std::shared_ptr<extra_tasks::EffectAggregatedTask>
        &task; // Use task_proxy to access task information.
    OperatorID operator_id; // Single operator with multiple conditional effects
    bool dynamic_splitting; // Indicates if we try variable-based with a backup
                            // of effect-based splitting
    bool variable_based_splitting; // Determines if we use auxillary variables
                                   // and full partioning
    ...
    std::vector<std::vector<DisjunctiveTransitionRelation>>
        var_based_effect_transitions;

    BDD all_exists_vars,
        all_exists_bw_vars; // Cube with variables to existentialize of all trs
    std::vector<BDD> exists_vars,
        exists_bw_vars; // Cubes per transitons relation for existentialize
    std::vector<BDD> all_swap_vars,
        all_swap_vars_p; // Swap variables from unprimed to primed
    ...
    void add_precondition_trs(
        const PreconditionsProxy &pre,
        std::vector<DisjunctiveTransitionRelation> &transition);
    void add_aux_condition_trs(
