...
UniformCostSearch::UniformCostSearch(SymController *eng,
                                     const SymParamsSearch &params) : UnidirectionalSearch(eng, params),
                                                                      parent(nullptr),
...
void UniformCostSearch::checkCutOriginal(Bucket &bucket, int g_val)
{
    ...
    for (Bdd &bucketBDD : bucket)
    {
        if (!p.top_k)
        {
            ...
            auto all_sols = perfectHeuristic->getAllCuts(this, bucketBDD, g_val, fw, engine->getLowerBound());
...
void UniformCostSearch::prepareBucket()
{
    if (!frontier.bucketReady())
    {
        ...
        if (p.top_k) {
            Bdd closed_states = !engine->get_states_on_goal_paths() * closed->getClosed();
        ...
        if (isAbstracted() || !lastStepCost || frontier.g() != 0)
        {
            ...
            DEBUG_MSG(cout << "Insert g=" << frontier.g() << " states into closed: " << nodeCount(frontier.bucket()) << " (" << frontier.bucket().size() << " bdds)" << endl;);
...
bool UniformCostSearch::stepImage(int maxTime, int maxNodes)
{
    ...
    if (!prepare_res.ok)
    {
        ...
        if (sTime() * 1000.0 > p.maxStepTime)
        {
            ...
            DEBUG_MSG(cout << "MAX STEP NODES CHANGED TO: " << p.maxStepNodes << " after truncating with " << sTime() << " seconds" << endl;);
        }
