...
class SymStateSpaceManager {
    ...
    void preimage_parallel(const Bdd &bdd, int nodeLimit, const std::vector <TransitionRelation>& trs, std::vector <Bdd> &res, int i, int k) const;
    void image_parallel(const Bdd &bdd, int nodeLimit, const std::vector <TransitionRelation>& trs, std::vector <Bdd> &res, int i, int k) const;
...
public:
    ...
    bool isOriginal() const {
        return static_cast<int>(relevant_vars.size()) == tasks::g_root_task->get_num_variables();
    }
    ...
    const SymParamsMgr getParams() const {
        return p;
    }
