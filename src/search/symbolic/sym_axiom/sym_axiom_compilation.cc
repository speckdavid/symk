...
namespace symbolic {
    ...
    bool SymAxiomCompilation::is_trivial_axiom(int axiom_id) const {
        int head = task.get_axioms()[axiom_id].get_effects()[0].get_fact().get_variable().get_id();
    ...
    int SymAxiomCompilation::get_axiom_level(int axiom_id) const {
        return task.get_axioms()[axiom_id].get_effects()[0].get_fact().get_variable().get_axiom_layer();
    }
