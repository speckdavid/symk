...
namespace symbolic {
    ...
    void SymVariables::init(const vector<int> &v_order) {
        ...
        ax_comp = std::shared_ptr<SymAxiomCompilation>(new SymAxiomCompilation(std::shared_ptr<SymVariables>(this)));
        if (task_properties::has_axioms(TaskProxy(*tasks::g_root_task))) {
            std::cout << "Creating Primary Representation for Derived Predicates..." << std::endl;
            ax_comp->init_axioms();
    ...
    std::vector<std::string> SymVariables::get_fd_variable_names() const {
        ...
        for (int v : var_order) {
            ...
            for (int j : bdd_index_pre[v]) {
                var_names[j] = tasks::g_root_task->get_variable_name(v) + "_2^" + std::to_string(exp);
                var_names[j + 1] =
                        tasks::g_root_task->get_variable_name(v) + "_2^" + std::to_string(exp++) + "_primed";
            }
        }
