...
string PlanForbidReformulatedTask::get_operator_name(int index,
  ...
  return parent->get_operator_name(parent_op_index, is_axiom);
}
...
vector<int> PlanForbidReformulatedTask::get_initial_state_values() const {
  ...
  for (int i = 0; i < get_num_variables(); i++) {
    if (i < parent->get_num_variables()) {
      result.push_back(parent->get_initial_state_values().at(i));
    } else if (i == parent->get_num_variables() ||
               i == parent->get_num_variables() + 1) {
      result.push_back(1);
    } else {
      result.push_back(0);
    }
