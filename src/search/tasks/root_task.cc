...
public:
    RootTask(
        vector<ExplicitVariable> &&variables,
        vector<vector<set<FactPair>>> &&mutexes,
        vector<MutexGroup> &&mutex_groups, vector<ExplicitOperator> &&operators,
        vector<ExplicitOperator> &&axioms, vector<int> &&initial_state_values,
        vector<FactPair> &&goals);
...
class TaskParser {
    ...
    vector<MutexGroup> read_mutexes(const vector<ExplicitVariable> &variables);
    vector<vector<set<FactPair>>> create_fd_mutexes(
...
vector<MutexGroup> TaskParser::read_mutexes(
    const vector<ExplicitVariable> &variables) {
        ...
        for (int i = 0; i < num_mutex_groups; ++i) {
            with_error_context(
                ...
                [&]() {
                    ...
                    */
                    string line = read_string_line("mutex group type");

                    int num_facts = -1;
                    bool detected_fw = true;

                    if (line == "fw") {
                        num_facts = read_nat("number of facts in mutex group");
                        lexer.confirm_end_of_line();
                    } else if (line == "bw") {
                        detected_fw = false;
                        num_facts = read_nat("number of facts in mutex group");
                        ...
                        if (line.find_first_not_of("0123456789") !=
                            string::npos) {
                            cerr
                                << "Expected 'fw', 'bw' or number of facts in mutex group, but got '"
                                << line << "' (no. " << lexer.get_line_number()
                                << ")." << endl;
                    ...
                    mutex_groups.emplace_back(
                        move(invariant_group), detected_fw);
                });
        }
        return mutex_groups;
    });
...
vector<vector<set<FactPair>>> TaskParser::create_fd_mutexes(
    const vector<MutexGroup> &mutex_groups,
    const vector<ExplicitVariable> &variables) {
...
shared_ptr<AbstractTask> TaskParser::read_task() {
    ...
    vector<MutexGroup> mutex_groups = read_mutexes(variables);
    vector<vector<set<FactPair>>> mutexes =
        create_fd_mutexes(mutex_groups, variables);
    ...
    */
    shared_ptr<RootTask> task = make_shared<RootTask>(
        move(variables), move(mutexes), move(mutex_groups), move(operators),
        move(axioms), move(initial_state_values), move(goals));
...
RootTask::RootTask(
    vector<ExplicitVariable> &&variables,
    vector<vector<set<FactPair>>> &&mutexes, vector<MutexGroup> &&mutex_groups,
    ...
    : variables(move(variables)),
      ...
      mutex_groups(move(mutex_groups)),
