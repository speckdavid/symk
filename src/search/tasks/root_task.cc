...
public:
  ...
  virtual void dump_mutexes(std::ostream &os) const override;
  virtual bool get_use_metric() const override;
  ...
  virtual FactPair get_operator_effect(int op_index, int eff_index,
                                       bool is_axiom) const override;
  virtual int
  convert_operator_index(int index,
  ...
  virtual void
  convert_state_values(vector<int> &values,
                       const AbstractTask *ancestor_task) const override;
};
...
vector<MutexGroup> read_mutexes(istream &in) {
...
create_fd_mutexes(const vector<MutexGroup> &mutex_groups,
                  const vector<ExplicitVariable> &variables) {
...
read_actions(istream &in, bool is_axiom, bool use_metric,
             const vector<ExplicitVariable> &variables) {
  int count;
  in >> count;
...
RootTask::RootTask(istream &in) {
  ...
  use_metric = read_metric(in);
  variables = read_variables(in);
  int num_variables = variables.size();

  mutex_groups = read_mutexes(in);
  mutexes = create_fd_mutexes(mutex_groups, variables);
...
void RootTask::dump_mutexes(std::ostream &os) const {
  os << mutex_groups.size() << std::endl;
  for (auto &mg : mutex_groups) {
    os << "begin_mutex_group" << std::endl;
    os << (mg.is_exactly_one() ? "exactly_one" : "mutex") << std::endl;
    os << (mg.detectedFW() ? "fw" : "bw") << std::endl;
    os << mg.getFacts().size() << std::endl;
    for (auto &fact : mg.getFacts()) {
      os << fact.var << " " << fact.value << std::endl;
    }
    os << "end_mutex_group" << std::endl;
  }
}

bool RootTask::get_use_metric() const { return use_metric; }
