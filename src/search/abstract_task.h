...
protected:
  ...
  void dump_metric(std::ostream &os) const;
  void
  dump_operator_pre_post_to_SAS(std::ostream &os, int pre, const FactPair &eff,
...
public:
  ...
  virtual bool get_use_metric() const = 0;
...
*/
  virtual void
  convert_state_values(std::vector<int> &values,
                       const AbstractTask *ancestor_task) const = 0;

  virtual void dump_to_SAS(std::ostream &os) const;
  virtual void dump_mutexes(std::ostream &os) const = 0;
