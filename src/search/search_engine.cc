...
get_successor_generator(const TaskProxy &task_proxy) {
...
SearchEngine::SearchEngine(const Options &opts)
    : status(IN_PROGRESS), solution_found(false), save_plans(true),
      task(tasks::g_root_task), task_proxy(*task), state_registry(task_proxy),
...
void SearchEngine::print_statistics() const {
  cout << "Bytes per state: " << state_registry.get_state_size_in_bytes()
       << endl;
}
...
void SearchEngine::add_options_to_parser(OptionParser &parser) {
  ...
  parser.add_option<int>("bound",
                         "exclusive depth bound on g-values. Cutoffs are "
                         "always performed according to "
...
static PluginTypePlugin<SearchEngine>
    _type_plugin("SearchEngine",
...
void collect_preferred_operators(
  ...
  if (!eval_context.is_evaluator_value_infinite(preferred_operator_evaluator)) {
    for (OperatorID op_id :
         eval_context.get_preferred_operators(preferred_operator_evaluator)) {
