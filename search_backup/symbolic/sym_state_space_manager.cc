...
void SymStateSpaceManager::zero_preimage(const Bdd &bdd, vector <Bdd> &res, int nodeLimit) const {
        ...
        if(Bdd::is_parallel_search()) {
            ...
            // Compute in parallel
            preimage_parallel(bdd, nodeLimit, transitions.at(0), cur_res, 0, transitions.at(0).size());
            // Append to result
...
void SymStateSpaceManager::zero_image(const Bdd &bdd, vector <Bdd> &res, int nodeLimit) const {
        ...
        if(Bdd::is_parallel_search()) {
            ...
            vector<Bdd> cur_res(transitions.at(0).size(), Bdd::BddOne());
            // Compute in parallel
            image_parallel(bdd, nodeLimit, transitions.at(0), cur_res, 0, transitions.at(0).size());
            // Append to result
            res.insert(std::end(res), std::begin(cur_res), std::end(cur_res));
        } else {
            for (const auto &tr : transitions.at(0)) {
...
void SymStateSpaceManager::cost_preimage(const Bdd &bdd, map<int, vector<Bdd>> &res,
        ...
        if(Bdd::is_parallel_search()) {
            for (auto trs : transitions) {
                ...
                // Compute in parallel
                preimage_parallel(bdd, nodeLimit, transitions.at(cost), cur_res, 0, transitions.at(cost).size());
                // Append to result
                res[cost].insert(std::end(res[cost]), std::begin(cur_res), std::end(cur_res));
            }
        } else {
            for (auto trs : transitions) {
                int cost = trs.first;
...
void SymStateSpaceManager::cost_image(const Bdd &bdd,
        ...
        if(Bdd::is_parallel_search()) {
            for (auto trs : transitions) {
                ...
                vector<Bdd> cur_res(transitions.at(cost).size(), Bdd::BddOne());
                // Compute in parallel
                image_parallel(bdd, nodeLimit, transitions.at(cost), cur_res, 0, transitions.at(cost).size());
                // Append to result
                res[cost].insert(std::end(res[cost]), std::begin(cur_res), std::end(cur_res));
            }
        } else {
...
void SymStateSpaceManager::preimage_parallel(const Bdd &bdd, int nodeLimit, const std::vector <TransitionRelation>& trs, std::vector <Bdd> &res, int i, int k) const {
    #ifdef DDLIBSYLVAN
        LACE_ME
    #endif
    if (k == 1) {
...
void SymStateSpaceManager::image_parallel(const Bdd &bdd, int nodeLimit, const std::vector <TransitionRelation>& trs, std::vector <Bdd> &res, int i, int k) const {
    #ifdef DDLIBSYLVAN
        LACE_ME
    #endif
    if (k == 1) {
...
SymParamsMgr::SymParamsMgr(const options::Options &opts) :
    ...
    if (mutex_type == MutexType::MUTEX_EDELETION && task_properties::has_conditional_effects(task_proxy)) {
