...
UniformCostSearch::UniformCostSearch(SymController *eng,
    ...
    : UnidirectionalSearch(eng, params),
      parent(nullptr),
      closed(make_shared<ClosedList>(params.top_k_plans)),
      estimationCost(params),
      estimationZero(params),
      lastStepCost(true) {}
...
void UniformCostSearch::checkCutOriginal(Bucket &bucket, int g_val) {
  ...
  for (Bdd &bucketBDD : bucket) {
    ...
    // bucketBDD *= perfectHeuristic->notClosed(); //Prune everything closed in
    // opposite direction
  }
}
