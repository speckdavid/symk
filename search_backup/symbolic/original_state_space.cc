...
OriginalStateSpace::OriginalStateSpace(SymVariables *v,
                                       const SymParamsMgr &params) : SymStateSpaceManager(v, params)
{
    initialState = vars->getStateBDD(tasks::g_root_task->get_initial_state_values());
...
void OriginalStateSpace::create_single_trs_parallel_recursive(std::vector <TransitionRelation>& trs, int i, int k) {
    if (k == 1) {
        trs.at(i).init();
        if (p.mutex_type == MutexType::MUTEX_EDELETION)
        {
            trs.at(i).edeletion(notMutexBDDsByFluentFw, notMutexBDDsByFluentBw, exactlyOneBDDsByFluent);
        }
    } else {
        #ifdef DDLIBSYLVAN
          LACE_ME
...
void OriginalStateSpace::init_mutex(const std::vector<MutexGroup> &mutex_groups)
{
    ...
    if (genMutexBDDByFluent)
    {
        ...
        {
            exactlyOneBDDsByFluent[i].resize(tasks::g_root_task->get_variable_domain_size(i));
                                    ...
                                    bool genMutexBDD, bool genMutexBDDByFluent, bool fw)
{
    ...
    if (genMutexBDDByFluent)
    {
        ...
        for (int i = 0; i < tasks::g_root_task->get_num_variables(); ++i)
        {
            notMutexBDDsByFluent[i].resize(tasks::g_root_task->get_variable_domain_size(i));
            for (int j = 0; j < tasks::g_root_task->get_variable_domain_size(i); ++j)
            {
    ...
    vector<vector<Bdd>> invariant_bdds_by_fluent(tasks::g_root_task->get_num_variables());
    for (size_t i = 0; i < invariant_bdds_by_fluent.size(); i++)
    {
        mBDDByVar.push_back(oneBDD());
        invariant_bdds_by_fluent[i].resize(tasks::g_root_task->get_variable_domain_size(i));
